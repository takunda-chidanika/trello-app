    ...
    return (
        <header>
            ...
            bg-gray-500/10 rounded-b-2xl`}>
                <div
                    className={`absolute top-0 left-0 w-full h-96 bg-gradient-to-br from-pink-400 to-[#0055D2]
                ...
                <div className={`flex items-center space-x-5 flex-1 justify-end w-full`}>
                    {/*    Search Bar */}
                    <form className={`flex items-center space-x-3 rounded-md 
                p-2 shadow-md bg-white flex-1 md:flex-initial`}>
            ...
            md:py-5`}>
                ...
                rounded-xl w-fit bg-white italic max-w-3xl text-[#0055D1] p-5`}>
                    <UserCircleIcon className={`inline-block h-10 w-10 text-[#0055D1] mr-1
                    ${loading ? "animate-spin" : "animate-pulse"}`}/>
                    {suggestion && !loading ? suggestion
                        : `GPT is summarising your tasks, please wait...`}
                </p>
            </div>
        </header>
    );
